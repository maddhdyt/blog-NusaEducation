...
@section('content')
    <div class="mb-6 flex justify-between items-center">
        <h2 class="text-xl font-heading font-bold text-[#0a1435] tracking-tight">Daftar Halaman</h2>
        <a href="{{ route('admin.pages.create') }}" class="inline-flex items-center px-4 py-2 bg-[#0a1435] border border-[#0a1435] text-white text-sm font-bold uppercase tracking-wider hover:bg-[#FDF6F0] hover:text-[#0a1435] transition-colors">
            ...
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="bg-[#FDF6F0]">
                        ...
                        <th class="py-4 px-6 text-xs font-bold text-[#0a1435] uppercase tracking-wider border-b border-[#0a1435]">Created</th>
                ...
                <tbody class="divide-y divide-[#0a1435]/10">
                    @forelse($pages as $page)
                        <tr class="hover:bg-[#FDF6F0] transition-colors group">
                            <td class="py-4 px-6">
                                <div class="text-sm font-bold text-[#0a1435]">{{ $page->title }}</div>
                                <div class="text-xs text-[#0a1435]/70">{{ Str::limit($page->excerpt, 50) }}</div>
                            </td>
                            ...
                            <td class="py-4 px-6 whitespace-nowrap text-right">
                                <div class="flex items-center justify-end gap-2">
                                    ...
                                    <form action="{{ route('admin.pages.destroy', $page) }}" method="POST" class="inline" onsubmit="confirmDelete(event, this, 'Hapus Halaman?', 'Halaman ini akan dihapus permanen!');">
                                        @csrf
                                        @method('DELETE')
